...
def test_claw_wide(channel):
    print(f"\n--- Testing Channel {channel} (CLAW - WIDE OPEN) ---")
    ...
    print("Sweeping slowly down to 60 degrees (closed limit)...")
    for a in range(90, 59, -1):
        pca.set_angle(channel, a)
        time.sleep(sweep_delay)
    time.sleep(1)
    
    print("Sweeping slowly up to 180 degrees (full open limit)...")
    for a in range(60, 181, 1):
        pca.set_angle(channel, a)
        time.sleep(sweep_delay)
    time.sleep(1)
...
# Test Claw (Channel 5) from 60 (closed) up to 180 (fully open)
test_claw_wide(5)
