...
def test_full_range_slow(channel):
    print(f"\n--- Testing Channel {channel} (SLOW MODE) ---")
    print("Moving to Center (90)...")
    pca.set_angle(channel, 90)
    time.sleep(2)
    
    # Slower sweep: increased delay from 0.03s to 0.08s per degree
    sweep_delay = 0.08
    
    print("Sweeping very slowly towards 0 degrees...")
    for a in range(90, -1, -1):
        pca.set_angle(channel, a)
        time.sleep(sweep_delay)
    time.sleep(1)
    
    print("Sweeping very slowly towards 180 degrees...")
...
# Test Wrist (Channel 3) in slow mode all the way to 0 and 180
test_full_range_slow(3)
