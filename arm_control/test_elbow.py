...
def test_safe_range(channel):
    print(f"\n--- Testing Channel {channel} ---")
    print("Moving to Center (90)...")
    pca.set_angle(channel, 90)
    time.sleep(2)
    
    # We will test 20 to 160 for the elbow to be safe, 
    # as elbows often have less physical clearance than the base.
    print("Sweeping slowly towards 20 degrees...")
    for a in range(90, 19, -1):
        pca.set_angle(channel, a)
        time.sleep(0.03)
    time.sleep(1)
    
    print("Sweeping slowly towards 160 degrees...")
    for a in range(20, 161, 1):
        pca.set_angle(channel, a)
        time.sleep(0.03)
    time.sleep(1)
    
    print("Returning to Center (90)...")
    for a in range(160, 89, -1):
        pca.set_angle(channel, a)
        time.sleep(0.03)
    time.sleep(1)
...
# Test Elbow (Channel 2) safely between 20 and 160 degrees
test_safe_range(2)
